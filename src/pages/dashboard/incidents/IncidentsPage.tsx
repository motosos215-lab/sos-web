...
export function IncidentsPage() {
  ...
  const loadIncidents = useCallback(
    async (nextPage: number, refreshing = false) => {
      if (rangeInvalid) {
        return;
      }
      setErrorMessage("");
      ...
      try {
        const [response, allResponse] = await Promise.all([
          getIncidents(filters, { page: nextPage, pageSize }, session),
          getIncidents(initialFilters, { page: 1, pageSize: 100 }, session),
        ]);
        ...
        if (response.success && response.data) {
          ...
          setErrorMessage(response.message || "No pudimos cargar los incidentes");
        }
  ...
  return (
    <div className="incidents-page">
      <header className="incidents-page__header">
        ...
        <div className="incidents-page__header-actions">
          <Button
            ...
            onClick={() => void loadIncidents(page, true)}
            type="button"
            variant="secondary"
          >
            <RefreshCw aria-hidden="true" size={16} /> Actualizar
          </Button>
          <Link className="button button--secondary" to="/dashboard/reportes">
            <Download aria-hidden="true" size={16} /> Exportar vista
          </Link>
          <Button onClick={clearFilters} type="button" variant="secondary">
            <RotateCcw aria-hidden="true" size={16} /> Limpiar filtros
          </Button>
        </div>
      </header>
      ...
      <form className="incidents-filters" noValidate onSubmit={handleSubmitFilters}>
        ...
        <label>
          <span>Estado</span>
          <select onChange={(event) => updateFilters({ status: event.target.value as IncidentFilters["status"] })} value={filters.status}>
        ...
        <label>
          <span>Severidad</span>
          <select
            onChange={(event) => updateFilters({ severity: event.target.value as IncidentFilters["severity"] })}
        ...
        <label>
          <span>Origen</span>
          <select onChange={(event) => updateFilters({ origin: event.target.value as IncidentFilters["origin"] })} value={filters.origin}>
            <option value="all">Todos</option>
            <option value="automatic_detection">Detección automática</option>
            <option value="manual_sos">SOS manual</option>
          </select>
        </label>
        <label>
          <span>Fecha desde</span>
          <input
            aria-describedby={rangeInvalid ? "incident-date-error" : undefined}
        ...
        <label>
          <span>Ordenar</span>
          <select onChange={(event) => updateFilters({ sortBy: event.target.value as IncidentFilters["sortBy"] })} value={filters.sortBy}>
        ...
        {rangeInvalid ? (
          <p className="incidents-filters__error" id="incident-date-error" role="alert">
      ...
      {isLoading ? (
        <section className="incidents-state" aria-busy="true">
          Cargando incidentes...
        </section>
      ) : null}
      {!isLoading && !result ? (
        <section className="incidents-state">
          <p>No pudimos cargar los incidentes</p>
          <Button onClick={() => void loadIncidents(1)} type="button">
            Reintentar
          </Button>
        </section>
      ) : null}
      {!isLoading && result && result.totalItems === 0 ? (
        <section className="incidents-state">
          <h2>{hasFilters ? "No encontramos incidentes con los filtros seleccionados" : "No hay incidentes registrados"}</h2>
          {hasFilters ? (
      ...
      {!isLoading && result && result.items.length > 0 ? (
        <>
              ...
              <tbody>
                {result.items.map((incident) => (
                  <tr key={incident.folio}>
                    ...
                    <td>{formatReadableDate(incident.occurredAt)}</td>
                    <td>{incident.driver.fullName}</td>
                    <td>
                      {incident.vehicle.alias} · {incident.vehicle.brand} {incident.vehicle.model}
                    </td>
                    <td>{incident.incidentType}</td>
                    <td>{getOriginLabel(incident.origin)}</td>
                    <td>
                    ...
                    <td>{incident.elapsedMinutes} min</td>
                    <td>{incident.locationLabel}</td>
                    <td>
          ...
          <div className="incidents-mobile-list">
            {result.items.map((incident) => (
              <article className="incident-mobile-card" key={incident.folio}>
                ...
                <IncidentStatusBadge status={incident.status} />
                <p>{incident.driver.fullName}</p>
                <p>
                  {incident.vehicle.alias} · {incident.vehicle.brand} {incident.vehicle.model}
                </p>
                <p>
                  {incident.incidentType} · {getOriginLabel(incident.origin)}
                </p>
                <p>
                  {formatReadableDate(incident.occurredAt)} · {incident.elapsedMinutes} min
                </p>
          ...
          <nav className="incidents-pagination" aria-label="Paginación de incidentes">
            <span>
              Mostrando {showingStart} a {showingEnd} de {result.totalItems} incidentes
            </span>
            <Button
              disabled={result.currentPage === 1}
              onClick={() => void loadIncidents(result.currentPage - 1)}
              type="button"
              variant="secondary"
            >
              Anterior
            ...
            {Array.from({ length: result.totalPages }, (_, index) => index + 1).map((pageNumber) => (
              <button
                aria-current={pageNumber === result.currentPage ? "page" : undefined}
                key={pageNumber}
                onClick={() => void loadIncidents(pageNumber)}
                type="button"
              >
                {pageNumber}
              </button>
