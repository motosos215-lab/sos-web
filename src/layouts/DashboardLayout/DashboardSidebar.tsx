...
import { dashboardNavItems } from "./dashboardNavigation";
  ...
  return (
    <aside className="dashboard-sidebar" aria-label="Navegación principal del dashboard">
      <div className="dashboard-sidebar__brand">
        <div className="dashboard-sidebar__logo" aria-hidden="true">MS</div>
        <div>
      ...
      <nav className="dashboard-sidebar__nav" aria-label="Secciones del dashboard">
        {dashboardNavItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className="dashboard-sidebar__icon" aria-hidden="true"><item.icon size={18} /></span>
      ...
      <section className="dashboard-sidebar__user" aria-label="Sesión activa">
        <div className="dashboard-avatar" aria-hidden="true">{getInitials(session.name)}</div>
        <div>
          <strong>{session.name}</strong>
          <span>{getRoleLabel(session.role)}</span>
          <small>{accountStatus} · Plan {session.plan}</small>
        </div>
        <Button onClick={onLogout} type="button" variant="secondary">Cerrar sesión</Button>
      </section>
    </aside>
  );
}
